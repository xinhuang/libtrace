...
public:
  ...
  failure(const Expected& e, Actual& a, const char* op, const char* at = "", const char* expr = "")
  {
    std::ostringstream os;
    os << at << expr << "\nexpected: "
       << "(" << demangle<Expected>() << ") " << e
       << " " << op << " actual: " << "(" << demangle<Actual>() << ") " << a;
    failure_ = os.str();
  }
...
inline void less(const Lhs& lhs, const Rhs& rhs, const char* at = "", const char* expr = "") 
{
  if (!(lhs < rhs)) 
  {
	throw failure(lhs, rhs, "<", at, expr);
  }
}
